...
export const listDonationDetails = (donationId) => async (dispatch, getState) => {
    try {
        ...
        const {data} = await api.get("/api/donations/" + donationId, config);
...
export const ListQuestion = (donationType = '') => async (dispatch) => {
    try {
        ...
        const {data} = await api.get(`/api/donations/questions/?donation_type=${donationType}`);
        dispatch({
...
export const listDonationResponses = (donationId) => async (dispatch, getState) => {
    try {
        ...
        const {data} = await api.get(`/api/donations/${donationId}/responses/`, config);
