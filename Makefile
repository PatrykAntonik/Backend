...
lint-check: ## Run lint check
	@echo "Running pylint..."
	$(COMPOSE) exec $(API_SERVICE) pylint --fail-under=8.0 donation transplantApp

lint: ## Run lint and attempt to fix issues
	@echo "Running pylint..."
	$(COMPOSE) exec $(API_SERVICE) ruff check donation transplantApp --fix

sort-check: ## Run sorting check
	@echo "Running Sort..."
	$(COMPOSE) exec $(API_SERVICE) isort -c donation transplantApp

sort: ## Run sorting
	@echo "Running Sort..."
	$(COMPOSE) exec $(API_SERVICE) isort donation transplantApp

format-check: ## Run formatter check
	@echo "Running formatter..."
	$(COMPOSE) exec $(API_SERVICE) black transplantApp donation --check

format: ## Run formatter
	@echo "Running formatter..."
	$(COMPOSE) exec $(API_SERVICE) black transplantApp donation
